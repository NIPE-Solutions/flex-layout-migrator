...
The transaction documentation is careful not to promise crash durability. Concurrent destination drift fails preflight, handled failures attempt rollback and cleanup, report writing occurs after and outside project application, and Git or a verified backup remains the durable recovery boundary. Rerun guidance distinguishes byte-idempotence for the same successful scope from an incorrect promise that unresolved cases or different invocations become identical. The native CSS merger is additive: smaller or empty incoming rule sets retain unmatched valid owned rules, so reruns do not garbage-collect stale owned CSS, remove its file, or act as a complete-project pruning mode.
...
Public prose was separately audited for automation, safety, stability, performance, versions, compatibility, privacy, and transaction claims. High-risk copy has exact assertions, while the claim audit explicitly says narrative prose is also human-reviewed and is not comprehensively proven merely by registry checks. The rerun ownership contract now executes the production stylesheet merger with an empty incoming set and requires the public guide to describe retained unmatched CSS, absence of garbage collection, and absence of a complete-project pruning mode. No project-wide performance, compatibility, or complete-automation claim is retained.

## 18. Implementation, design, accessibility, performance, privacy, and security review

The implementation and final whole-branch reviews found two Important issues and no Critical issues. First, the verified-example `<pre>` elements used local horizontal scrolling but, unlike the shared code-block component and other scrollers, were not keyboard focusable. An adversarial all-route Axe sweep reproduced `scrollable-region-focusable` on nine affected nodes at desktop width and 18 at mobile width. A focused regression now requires every verified-example code region to have `tabindex="0"`; the production component applies it to input, output, and optional CSS regions. Second, rerun prose incorrectly implied that a current plan with no owned rules could remove prior owned output; the production merger retains those unmatched rules, and the guide plus production-bound contract now state that exact additive behavior.

The tracked Axe browser gate now derives the complete public route matrix from the generated sitemap, audits all 28 home, documentation, and legal routes under both desktop and mobile Playwright projects, and fails on either Critical or Serious findings. A deliberate scroller mutation proved that this gate reports `scrollable-region-focusable` on `/docs/examples`; the focused component regression remains in place. The post-fix 56-run matrix produced zero Critical or Serious findings, while the separate layout audit produced zero page-level horizontal-overflow findings.
