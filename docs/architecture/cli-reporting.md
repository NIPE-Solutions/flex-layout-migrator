...
Angular parse errors return code `1`. A malformed file is never written. For folder input, other files may have completed before a later parse or I/O failure; the report records every result collected before the failure and the terminal error identifies the failed operation.
